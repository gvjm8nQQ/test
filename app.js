...
const c7Score = {VI:100, I:76, C:48, N:14, U:28};
const c7Tone = {VI:'hard', I:'important', C:'considered', N:'no', U:'unknown'};
const verificationLabel = {
  'verified-official-cds':'CDS已核验', 'pending-official-cds':'CDS待核验',
  'estimated-needs-official-tuition-check':'费用待官网核验', 'estimated-needs-CDS-or-Scorecard-check':'录取率待核验',
  'needs-official-awards-check':'诺奖待官网核验', 'source-linked':'来源链接待复核'
};
...
function init() {
  ...
  renderChips(); renderVerificationSummary(); render();
}
...
function renderVerificationSummary(){
  ...
  const cdsVerified = data.filter(s => s.verification.cdsC7 === 'verified-official-cds').length;
  const costVerified = data.filter(s => s.verification.cost === 'verified-official-tuition').length;
  if ($('verificationSummary')) $('verificationSummary').innerHTML = `<strong>${total}</strong> 所学校批量核验矩阵：CDS C7 已核验 <strong>${cdsVerified}</strong> 所，费用官网精确核验 <strong>${costVerified}</strong> 所；其余学校在详情页给出待核验字段和直达来源。`;
}
function render(){
...
function openDetail(id){
  ...
  $('dialogContent').innerHTML = `<section class="detail"><h2>${s.name} · ${s.zh}</h2><p class="meta">${state.level} · ${s.type} · ${s.city} · ${s.cityType}</p><div class="detail-grid"><div class="box"><h3>申请画像与录取权重</h3><div class="bars">${bar('课程难度',95,'hard')}${bar('GPA',88,'')}${bar('标化/语言',state.level==='本科'?75:82,'')}${bar('文书/PS',92,'hard')}${bar('推荐信',86,'')}${bar('科研/实习',state.level==='本科'?70:94,'hard')}${bar('面试/作品集',s.majors.includes('艺术')||s.majors.includes('影视传媒')?90:55,'')}</div><p><strong>GPA判断：</strong>${gpaNote}</p><p><strong>申请建议：</strong>${s.fit}校；若大陆普高/国际高中申请本科，建议把 ${s.majors.slice(0,3).join('、')} 相关课程、竞赛、科研或作品集中做出一条清晰主线。硕士申请则优先匹配项目先修课、量化/编程能力、科研或实习成果。</p></div><div class="box"><h3>CDS C7 学校真实录取因素</h3>${renderCdsC7(s)}<p class="sources">${s.cdsC7.evidence}</p></div><div class="box"><h3>学校与城市画像</h3><p><strong>强势专业：</strong>${s.strengths}</p><p><strong>城市样子：</strong>${s.vibe}</p><p><strong>费用：</strong>${fmtMoney(cost)}/年，通常包括学费、住宿餐饮、保险、书本和个人开销的估算；不等于拿到奖学金后的净价。</p><p><strong>排名：</strong>U.S. News National Universities #${s.usNews}；QS World University Rankings ${fmtQs(s.qs)}。</p><p><strong>诺奖：</strong>${s.nobel} 位（学校口径/关联口径可能不同，申请时建议以学校官方 Nobel 页面复核）。</p></div></div><div class="box sources"><h3>数据来源与口径</h3><p>${s.source}</p><p class="source-links"><a href="${s.sourceLinks.scorecard}" target="_blank" rel="noopener">College Scorecard</a><a href="${s.sourceLinks.usNews}" target="_blank" rel="noopener">U.S. News</a><a href="${s.sourceLinks.qs}" target="_blank" rel="noopener">QS</a><a href="${s.sourceLinks.cds}" target="_blank" rel="noopener">官方CDS C7检索</a><a href="${s.sourceLinks.tuition}" target="_blank" rel="noopener">官方费用检索</a><a href="${s.sourceLinks.nobel}" target="_blank" rel="noopener">官方诺奖检索</a><a href="${s.sourceLinks.official}" target="_blank" rel="noopener">学校官网检索</a></p><div class="verification-grid"><span>排名：${verificationLabel[s.verification.rankings]}</span><span>费用：${verificationLabel[s.verification.cost]}</span><span>录取率：${verificationLabel[s.verification.admitRate]}</span><span>诺奖：${verificationLabel[s.verification.nobel]}</span><span>CDS C7：${verificationLabel[s.verification.cdsC7]}</span></div><p>提示：录取率、费用、排名每年变化。本工具用于初筛和沟通，不替代学校官网、项目官网和 Common Data Set 的最终核验。</p></div></section>`;
...
function renderCdsC7(s){
  const labels = window.C7_LABELS || {};
  const fields = window.C7_FIELDS || [];
  return `<div class="bars cds-bars">${fields.map(([key,label]) => { const rating = s.cdsC7.ratings[key] || 'U'; return `<p><span>${label}</span><span class="bar ${c7Tone[rating]}"><i style="width:${c7Score[rating]}%"></i></span><em>${labels[rating] || rating}</em></p>`; }).join('')}</div><p><strong>CDS年份：</strong>${s.cdsC7.year} · <a href="${s.cdsC7.source}" target="_blank" rel="noopener">查看来源</a></p>`;
}
