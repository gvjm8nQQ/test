...
window.SCHOOL_DATA = SCHOOLS_100.map(([id,name,zh,city,state,type,cityType,preset,usNews,qs,nobel])=>({
  ...
  sourceLinks:{scorecard:`https://collegescorecard.ed.gov/search/?search=${encodeURIComponent(name)}`, usNews:`https://www.usnews.com/best-colleges/search?schoolName=${encodeURIComponent(name)}`, qs:`https://www.topuniversities.com/universities?search=${encodeURIComponent(name)}`, official:`https://www.google.com/search?q=${encodeURIComponent(name+' admissions tuition common data set')}`}
}));
